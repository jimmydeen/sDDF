...
int give_multi_char(uintptr_t drv_buffer, int drv_buffer_len) {
    for (int curr_client = 0; curr_client < NUM_CLIENTS; curr_client++) {
        ...
        memcpy(buffer, drv_buffer, drv_buffer_len);
...
int give_single_char(int curr_client, uintptr_t drv_buffer, int drv_buffer_len) {
...
int give_char(int curr_client, uintptr_t drv_buffer, int drv_buffer_len) {
    if (multi_client == 1) {
...
void handle_rx() {
    ...
    if (mode == RAW_MODE) {
       ...
       if (chars[0] == '@') {
            ...
            give_char(client, buffer, buffer_len);
       }
    }
